...
// This file is adapted from `sw/legacy/common/demo/ethernet/ksz8851.c`, but
...
static void ksz8851_write_mac(spi_t *spi, uint8_t addr[6]) {
...
bool ksz8851_output(struct netif *netif, struct fbuf *buf) {
  ...
  if (buf->len != 0) {
    spi_tx(spi, buf->payload, buf->len);
  }
