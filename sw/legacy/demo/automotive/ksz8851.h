...
// This file is adapted from `sw/legacy/common/demo/ethernet/ksz8851.h`, but
...
struct netif {
  // SPI reference to be used for data transfer
  spi_t *spi;
  // Maximum transfer units, in bytes
  uint16_t mtu;
  // Link-level hardware MAC source address. Assumed 6 bytes.
  uint8_t hwaddr[6];
};

// Stores information about a frame to send
struct fbuf {
  void *payload;
  uint16_t len;
};
